...
<template>
  <section class="page_box">
    <div id="pageBox" v-show="this.$store.state.movieSearch.movie_search_list['page'] > 1">
...
export default {
  ...
  computed: {
    ...
    page () {
      return this.$store.state.movieSearch.movie_search_list.pageSum
    }
  },
  methods: {
    addEllipsis () {
      ...
      this.oBox.addEventListener('click',(e) => {
        ...
        if (target.className.indexOf("item") > -1) {
          if (this.option.currentPage === target.innerText) {
            ...
            this.bus.$emit('http_movie_search', this.$route.query.kw,this.option.currentPage)
            window.scroll(0,0)
          }
        }
      }, false)
  ...
  mounted () {
    ...
    this.addEllipsis()
    this.addClick()
    this.bus.$on('http_movie_searchs', (val1,val2) => {
      this.redict()
    })
  }
}
